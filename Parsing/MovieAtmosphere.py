...
def set_up(driver):
    ...
    input_field.send_keys(
        "You are MovieAtmosphereDescriptor,an AI who is skilled in emotion and atmosphere description of movies.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys(
        "Your task is to describe atmosphere of movie that will be provided to you and to give appropriate plot tags. Also you should recommend books or games in the same atmosphere.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys("Silently do the following:")
    input_field.send_keys(
        "1) Start with '/start/Atmosphere:' word. In this section you are describing atmosphere of the movie. What was an inspiration of an author of the game if you know. If the movie is part of franchise do not mention that the movie's inspiration was in the franchise.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys("End the section with '/end/Atmosphere'.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys(
        "2) Start with '/start/Tags:' In this section you are describing plot tags of the movie. Tags should be separate metadata key words of the movies.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys("End the section with '/end/Tags'.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys(
        "3)Start with '/start/RecommensationsGame:' In this section you are giving games recommendation as many as possible. Movies should be in the same atmosphere as the game. If you can't give a recommendation type 'None'")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys("End the section with '/end/RecommensationsGame'.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys(
        "4)Start with '/start/RecommensationsBook:' In this section you are giving a book recommendation as many as possible. Books should be in the same atmosphere as the movie. If you can't give a recommendation type 'None'")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys("End the section with '/end/RecommensationsBook'.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys("Restrictions:")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys("You can not describe plot of the movie.")
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys(
    ...
    input_field.send_keys(Keys.SHIFT, Keys.ENTER)
    input_field.send_keys("Start when I send you next title of a movie.")
    time.sleep(5)
    input_field = driver.find_xpath(
...
def enter_open_ai():
    ...
    driver.preset()
    time.sleep(2)
    log_in = driver.find_xpath('//*[@id="__next"]/div[1]/div/div[4]/button[1]')
    time.sleep(3)
    log_in.click()
...
def df_update():
    ...
    input_field = driver.find_xpath(
        '// *[ @ id = "__next"] / div[1] / div[1] / main / div[2] / form / div / div[2] / textarea')
    input_field.send_keys(Keys.ENTER)
    df_games = pd.read_csv(r'C:\Users\misha\Pet-project\Parsing\movies_dataset.csv', lineterminator='\n')
    ...
    for i in df_games['title']:
        ...
        row += 1
        df_games.to_csv(r'C:\Users\misha\Downloads\movies_dataset.csv', index=False,line_terminator='\n',encoding='utf-8')
...
def scrap():
    ...
    time.sleep(5)
    input_field = driver.find_xpath(
        '//*[@id="__next"]/div[1]/div[2]/div/div/nav/div/div/a[2]/div[1]')
    time.sleep(5)
    input_field.click()
    df_games = pd.read_csv(r'C:\Users\misha\Downloads\movies_dataset.csv', lineterminator='\n')
    time.sleep(5)
    ...
    for i in df_games['title'][row:]:
        ...
        df_games.to_csv(r'C:\Users\misha\Downloads\movies_dataset.csv', index=False, line_terminator='\n',
                        encoding='utf-8')
# //*[@id="__next"]/div[1]/div[2]/div/div/nav/div/div/a[2]/div[1] //*[@id="__next"]/div[1]/div[1]/main/div[1]/div/div/div/div[18]/div/div[2]/div[1]/div/div
continue_parsing()